...
# Путь к модели
MODEL_PATH = "models/efficientnet_blood_model.pth"
...
# Тёмная тема + стиль
st.set_page_config(page_title="Клетки Крови", layout="wide")
...
# Заголовок
st.markdown('<div class="main-title">Клетки крови</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Загружайте фото и узнайте, какой вид крови у Вас</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    **Точность:** **87.08%**  
    **Классы:** 4 клетки крови   
    **Фреймворк:** PyTorch + torchvision  
    **Обучена на:** Blood Cell Images 
    """)
    
    st.subheader("Распознаваемые клетки:")
    st.write(", ".join([f"**{c}**" for c in CLASSES]))

    # Добавленное изображение
    st.subheader("Кривая обучения и метрики")
    st.image("images/training_history_blood.png", caption="История обучения модели", width=800)

# Футер
st.markdown("""
<div class="footer">
    Разработано с ❤️ | 2025
</div>
""", unsafe_allow_html=True)